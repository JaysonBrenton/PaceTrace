      ...
      <div className="space-y-10">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {mockMetrics.map((metric) => (
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-caps-wide text-muted-foreground/70">
                {metric.label}
              </p>
            ...
            <ul className="mt-4 space-y-3 text-sm text-muted-foreground">
              {["LiveRC", "Manual telemetry", "Coaching notes"].map((queue) => (
                <li key={queue} className="flex items-center justify-between rounded-xl border border-border/60 bg-card/70 px-4 py-3">
                  <span>{queue}</span>
                  <span className="text-xs uppercase tracking-caps-tight text-accent">Healthy</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="rounded-2xl border border-border/70 bg-background/80 p-5 lg:col-span-3">
            <h2 className="text-sm font-semibold uppercase tracking-caps text-muted-foreground">
              Crew radar
            </h2>
            <div className="mt-4 space-y-4">
              {["Pit lane", "Spotter deck", "Data review"].map((station) => (
                <div key={station} className="rounded-xl border border-border/60 bg-card/70 p-4">
                  <p className="text-xs uppercase tracking-caps-tight text-muted-foreground/70">{station}</p>
