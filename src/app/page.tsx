...
export default function Home() {
  return (
    <main className="relative isolate overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-72 bg-[radial-gradient(circle_at_top,hsla(var(--color-accent)/0.22),transparent_70%)]"
      />
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-16 px-4 pb-16 pt-10 sm:px-6 lg:px-8">
        <header className="flex items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.32em] text-muted-foreground">Pacetrace</p>
        ...
        <section className="grid gap-12 pb-6 pt-4 md:grid-cols-[minmax(0,_1fr)_minmax(0,_420px)] md:items-center" id="features">
          <div className="space-y-8">
            <span className="inline-flex items-center rounded-full border border-border/80 bg-card/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.32em] text-muted-foreground">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm">
              <Link
                href="/login"
                className="inline-flex items-center justify-center rounded-lg bg-accent px-5 py-3 font-semibold text-background transition hover:bg-[hsl(var(--color-accent-muted))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
            ...
            <dl className="grid gap-6 text-sm sm:grid-cols-3">
              <div className="rounded-2xl border border-border/70 bg-card/60 p-4">
                <dt className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/80">Live metrics</dt>
                <dd className="mt-3 text-2xl font-semibold text-foreground">Sub-200ms lap deltas</dd>
              </div>
              <div className="rounded-2xl border border-border/70 bg-card/60 p-4">
                <dt className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/80">Insights</dt>
                <dd className="mt-3 text-2xl font-semibold text-foreground">Auto-tagged stumbles</dd>
              </div>
              <div className="rounded-2xl border border-border/70 bg-card/60 p-4">
                <dt className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/80">Admin</dt>
                <dd className="mt-3 text-2xl font-semibold text-foreground">Quota-aware tenants</dd>
              </div>
            </dl>
          </div>

          <div className="relative rounded-[1.75rem] border border-border/60 bg-card/80 p-8 shadow-glow">
            <div className="absolute inset-0 -z-10 rounded-[1.75rem] bg-[radial-gradient(circle_at_top_right,hsla(var(--color-accent)/0.25),transparent_65%)]" aria-hidden />
            <div className="space-y-6 text-sm text-muted-foreground">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/70">Heat tracker</p>
              ...
              <div className="space-y-3">
                {["Lap 18", "Lap 19", "Lap 20"].map((lap, index) => (
                  <div key={lap} className="flex items-center justify-between rounded-xl border border-border/60 bg-background/40 px-4 py-3">
                    <div>
                      <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground/70">{lap}</p>
                      <p className="text-base font-semibold text-foreground">{index === 1 ? "Fast lap" : "Consistent"}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground/70">Delta</p>
              ...
              <div className="rounded-xl border border-border/60 bg-background/40 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/70">Coach note</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {["Health & readiness", "LiveRC import queue", "Tenant permissions"].map((item) => (
              <div key={item} className="rounded-2xl border border-border/60 bg-card/70 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground/70">{item}</p>
